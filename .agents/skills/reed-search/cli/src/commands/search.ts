...
const FALLBACK_LOCATIONS = ["London", "Manchester", "Glasgow", "Remote"]

/**
 * Resolve which locations to search when --location is omitted: the `defaults`
 * from config/uk-cities.json (the fork's single source of truth). Falls back to
 * a built-in UK set if the config is missing or unreadable.
 */
...
function buildUrl(opts: SearchOpts, location: string): string {
  const params = new URLSearchParams()
  if (opts.query) params.set("keywords", opts.query)
  if (location) params.set("location", location)
  if (opts.fullTime) params.set("fullTime", "true")
  if (opts.permanent) params.set("permanent", "true")
  params.set("sortby", "DisplayDate")
...
export async function runSearch(opts: SearchOpts): Promise<number> {
  try {
    ...
    for (const location of locations) {
      const html = await htmlFetch(buildUrl(opts, location))
